...
#gagliardo----
 ...
 is_geo <- TRUE
 data_dfs_gag <- group_vels(data_gag,
                        geo = is_geo,
                        verbose = TRUE,
                        parallelize = TRUE
 )
...
g_metr_gag %>% group_by(set) %>% summarise(n_timesteps = n(), N = mean(N)) #N; mean of N, splits data into set, summaries sets. n() number of elements. maxtime - meantime
 ...
 data_gag <- add_velocities(data_list = data_dfs_gag,
                                 geo = is_geo,
                                 verbose = TRUE,
                                 parallelize = TRUE,
                                 add_coords = FALSE # could be set to FALSE if the relative positions of neighbors are not needed
...
all_data <- rbind(fruitbat_metrics, santos_metrics, spearBat_metrics, gag_metrics)
